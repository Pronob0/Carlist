...
class DashboardController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $data['currenct_balance'] = round(auth()->user()->balance , 2);
        $data['total_deposit'] = round(Deposit::where('user_id', auth()->user()->id)->where('status', 'complete')->sum('amount'), 1);
        $total = Transaction::where('user_id', auth()->user()->id)->where('type', '+')->sum('amount');
        $minus = Transaction::where('user_id', auth()->user()->id)->where('type', '-')->sum('amount');
        $data['total_transaction'] = round(Transaction::where('user_id', auth()->user()->id)->where('remark', 'withdraw_money')->sum('amount'), 2);
        $data['total_tickets'] = auth()->user()->tickets()->count();
        $data['total_bid'] = auth()->user()->bids()->count();
        $data['total_bid_amount'] = round(auth()->user()->bids()->sum('bid_amount') , 2);
        $data['win_product'] = Order::where('winner_id', auth()->user()->id)->count();
        $data['curr'] = $curr;
        $data['total_loose_bid'] = auth()->user()->bids()->where('status', 3)->count();
        $transaction = Transaction::where('user_id', auth()->user()->id)->latest()->take(6)->get();
        $data['transactions'] = TransactionResource::collection($transaction)->response()->getData(true);
