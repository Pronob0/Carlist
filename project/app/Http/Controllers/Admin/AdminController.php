...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        $deposit = collect([]);
        $currency = Currency::where('default', 1)->first()->rate;
        Deposit::with('currency')->get()->map(function ($q) use ($deposit, $currency) {
            $deposit->push((float) ($q->amount * $currency));
        });
        $data['totalDeposit'] = round($deposit->sum(), 2);

        $withdraw = collect([]);
        Withdrawals::with('currency')->get()->map(function ($q) use ($withdraw, $currency) {
            $withdraw->push((float)($q->amount * $currency));
        });
