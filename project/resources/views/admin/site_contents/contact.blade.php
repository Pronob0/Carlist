               ...
               <div class="card-body">
                    <form action="{{route('admin.frontend.content.update',$section->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            @foreach(json_decode($section->content->phone) as  $phone)
                            <div class="form-group col-md-6">
                                <label for="">@lang('Phone No.')</label>
                                <input type="text" name="phone[]" class="form-control" placeholder="@lang('Phone No.')" value="{{$phone}}" required>
                            </div>

                            @endforeach
                            
                            
                            @foreach(json_decode($section->content->email) as  $email)
                            <div class="form-group col-md-6">
                                <label for="">@lang('Email')</label>
                                <input type="text" name="email[]" class="form-control" placeholder="@lang('Email')" value="{{ $email }}" >
                            </div>
                            @endforeach
                            ...
                            <div class="form-group col-md-12">
                                <label for="">@lang('Map Link')</label>
                                <input type="text" name="map_link" class="form-control" placeholder="@lang('Map Link')" value="{{@$section->content->map_link}}" required>
                            </div>
