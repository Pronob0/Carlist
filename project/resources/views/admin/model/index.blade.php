               ...
               @forelse ($models as $item)
                   <tr>
                        ...
                        <td data-label="@lang('Action')" class="text-right">
                           <a href="javascript:void()" class="btn btn-primary approve btn-sm edit mb-1" data-route="{{route('admin.model.update',$item->id)}}" data-item="{{$item}}"  data-toggle="tooltip" title="@lang('Edit')"><i class="fas fa-edit"></i></a>
...
<div class="modal fade" id="removeMod" tabindex="-1" role="dialog">
   <div class="modal-dialog" role="document">
      <form action="{{route('admin.model.destroy')}}" method="POST">
         @method('DELETE')
         @csrf
...
@push('script')
    <script>
       ...
       $('.edit').on('click',function () { 
          ...
          $('#edit').find('select[name=brand_id]').val(data.brand_id)
          $('#edit').modal('show')
       })
